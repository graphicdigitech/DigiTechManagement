...
// @Route     http://localhost:5000/api/employee/
// @Access    Private
// const OldcreateEmployee = async (req, res) => {
//   try {
//     const {
//       employeeName,
//       employeeEmail,
...
//       return res.status(400).json({
//         err: "Employee with this email already exists in our  employees database.You can recover from there",
//       });
//     }
...
const createEmployee = async (req, res) => {
  try {
    const {
      ...
      employeePassword,
      employeeSalary,
      employeeRoles = [], // roles array expected from frontend
      employeeTimeIn,
      employeeTimeOut,
      employeeallowances,
    } = req.body;
    ...
    // Converts "HH:mm" into a Karachi-based Date object
    const createDateWithTime = (time) => {
      const [hours, minutes] = time.split(":").map(Number);
      const today = new Date();
      ...
      return new Date(`${year}-${month}-${day}T${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:00+05:00`);
    };

    // Input Validation
    if (!employeeName || !nameRegex.test(employeeName))
      return res.status(400).json({ err: "Invalid Employee Name. Must contain at least 3 letters or spaces." });

    if (!employeeEmail || !emailRegex.test(employeeEmail))
      return res.status(400).json({ err: "Invalid Email Format." });

    if (!employeePassword || !passwordRegex.test(employeePassword))
      return res.status(400).json({ err: "Password must be 8+ chars, include uppercase, lowercase, number, special char." });

    if (!employeeSalary || !numberRegex.test(employeeSalary))
      return res.status(400).json({ err: "Salary must be a numeric value." });
    ...
    if (employeeTimeIn && employeeTimeOut) {
      ...
      if (checkInDate >= checkOutDate) {
        return res.status(400).json({ err: "Check-in time must be before check-out time." });
      }
    }

    // Check if employee already exists in deleted employees
    const existingDeleted = await deletedEmployeeModel.findOne({ employeeEmail });
    if (existingDeleted) {
      return res.status(400).json({
        err: "This email exists in deleted employees. Please recover or use a different email.",
      });
    }

    // Validate roles from DB (all roles must exist)
    const uniqueRoleIds = [...new Set(employeeRoles.map((r) => r.toString()))];
    const roles = await RoleModel.find({ _id: { $in: uniqueRoleIds } });

    if (roles.length !== uniqueRoleIds.length) {
      return res.status(400).json({ err: "One or more selected roles do not exist." });
    }
...
<tr>
    <td style="padding: 0;">
        <table role="presentation" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 0; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            ...
            <tr>
                <td style="padding: 40px 30px;">
                    ...
                    <p style="color: #333333; font-size: 16px; margin-bottom: 20px;">Dear ${employeeName},</p>
                    <p style="color: #333333; font-size: 16px; margin-bottom: 20px;">
                        It is with great pleasure that we welcome you to DigiTech! We are excited to have you join us and are confident that your skills and expertise will play a significant role in our ongoing success. Your contributions will strengthen our team and help us achieve our shared goals.
                    </p>
                    <h2 style="color: #003366; font-size: 20px; margin-top: 30px; margin-bottom: 15px;">Your Journey Starts Here</h2>
                    <table role="presentation" style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                        <tr>
                    ...
                    <p style="color: #333333; font-size: 16px; margin-bottom: 20px;">
                        Our HR Team is here to assist you during your onboarding process. Should you have any questions or require support, feel free to reach out to us at 
                        <a href="mailto:graphicdigitechdt@gmail.com" style="color: #003366; text-decoration: none;">graphicdigitechdt@gmail.com</a>.
                    </p>
                    <p style="color: #333333; font-size: 16px; margin-bottom: 20px;">
                        We eagerly look forward to witnessing your impact as you embark on this exciting new chapter with us.
                    </p>
            ...
            <tr>
                <td style="background-color: #f0f4f8; padding: 30px; text-align: center;">
                    ...
                    <a href="#" style="display: inline-block; margin: 0 10px;"><img src="https://via.placeholder.com/40x40?text=TW" alt="Twitter" style="width: 40px; height: 40px;"/></a>
                    <a href="#" style="display: inline-block; margin: 0 10px;"><img src="https://via.placeholder.com/40x40?text=IG" alt="Instagram" style="width: 40px; height: 40px;"/></a> -->
                    <p style="color: #666666; font-size: 12px; margin-top: 20px;">© 2024 DigiTech. All rights reserved.</p>
                </td>
            </tr>
        </table>
    </td>
      ...
      await SendMail(
        employeeEmail,
        "Welcome to the PrimeVertex!",
        `Hi ${employeeName}, welcome aboard!`,
        emailHTML
      );
    } catch (emailErr) {
    ...
    const newEmployee = await employeeModel.create({
      ...
      employeePassword: hashedPassword,
      employeeSalary,
      employeeRoles: uniqueRoleIds, // Only use roles passed by frontend
      employeeTimeIn,
      employeeTimeOut,
      employeeallowances,
    });
    ...
    const activeLeaveTypes = await LeaveTypeModel.find({ leaveTypeStatus: "active" });
    newEmployee.leaveBalances = activeLeaveTypes.map((leaveType) => ({
      leaveTypeId: leaveType._id,
      allowedLeaves: leaveType.allowedLeaves,
      currentBalance: leaveType.allowedLeaves,
    }));
