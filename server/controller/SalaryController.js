...
// This function handles the creation of a salary record for an employee
// It includes validation, attendance checks, salary calculations, email sending, and MongoDB transaction management
// It also ensures that all date/time processing is aligned to the Asia/Karachi timezone (UTC+5)
...
const createSalary = async (req, res) => {
  ...
  try {
    ...
    const startDate = moment.tz({ year, month: month - 1, day: 1 }, "Asia/Karachi").startOf("day").toDate();
    const endDate = moment.tz({ year, month: month - 1 }, "Asia/Karachi").endOf("month").endOf("day").toDate();
    const totalDaysInMonthCalculated = moment(endDate).date();
    ...
    if (loggedDays < Number(totalWorkingDays)) {
      return res.status(400).json({
        err: `Attendance is not fully logged for ${selectedMonth}. Only ${loggedDays} out of ${totalWorkingDays} working days are recorded.`,
      });
    }

    const onTimeDays = attendanceRecords.filter(r => r.status === "On Time").length;
    const lateDays = attendanceRecords.filter(r => r.status === "Late").length;
    const absentDaysFromAttendance = attendanceRecords.filter(r => r.status === "Absence").length;

    if (
      Number(daysOnTime) !== onTimeDays ||
      Number(daysLate) !== lateDays ||
      Number(absentDays) !== absentDaysFromAttendance
    ) {
      return res.status(400).json({
        err: `Attendance data mismatch. Expected OnTime: ${onTimeDays}, Late: ${lateDays}, Absent: ${absentDaysFromAttendance}`,
      });
    }

    // --- DUPLICATE CHECK ---
    const existingSalary = await SalaryModel.findOne({
      selectedMonth: moment.tz({ year, month: month - 1, day: 1 }, "Asia/Karachi").startOf("day").toDate(),
      employeeId,
    });
    ...
    await SendMail(
      employeeEmail,
      `Salary Statement for ${selectedMonth}`,
      `Dear ${employee.employeeName}, your salary breakdown is attached.`,
      "<HTML_TEMPLATE>" // Use your full email template here.
    );
      ...
      [
        {
          ...
          totalDeduction: Number(totalDeduction),
          remarks,
          totalAttendanceRecordDays: loggedDays,
        },
      ],
      { session }
    );
